...
describe("PostgresDatabaseConnection", () => {
  ...
  it("should execute transaction successfully", async () => {
    ...
    connection = PostgresDatabaseConnection.getInstance();
    const result = await connection.transaction(mockCallback);
  ...
  it("should handle transaction errors and rollback", async () => {
    ...
    connection = PostgresDatabaseConnection.getInstance();
    await expect(connection.transaction(mockCallback)).rejects.toThrow(
      "Transaction failed"
    );
  ...
  it("should handle connection errors in transaction", async () => {
    const connectionError = new Error("Connection failed");
    mockConnect.mockRejectedValueOnce(connectionError);

    connection = PostgresDatabaseConnection.getInstance();
    await expect(connection.transaction(async () => {})).rejects.toThrow(
      "Connection failed"
    );
  });
});
